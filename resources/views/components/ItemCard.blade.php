<div class="flex flex-col w-full border-2 border-solid
">
    <a href="/item/{{ $product->id }}">
        ...
        <div class="p-2">
            ...
            <div class="py-5">
                <div class="text-sm text-gray-500 font-bold">{{ $product->brand }}</div>
                <div class="text-gray-800 font-bold">{{ $product->name }}</div>
            </div>
            <form action="/cart/add/{{ $product->id }}">
                <button type="submit" class="w-full p-2 border-1 border-solid hover:cursor-pointer hover:bg-white hover:text-gray-800">
                    Añadir al carrito
                </button>
            </form>
        </div>
